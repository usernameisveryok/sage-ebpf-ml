...
/*
 * xdp_ml_v2.c — XDP in-kernel MLP inference with three operator-level
 *               innovations for real-time network intrusion detection.
 *
 * Architecture:  quantized MLP [12 → 32 → 32 → 4]
 * Approach:      Tail-call chain (4 stages) + three novel operators.
 *
 * ╔══════════════════════════════════════════════════════════════════╗
 * ║  INNOVATION 1 — Early-Exit Tail-Call Bypass                     ║
 * ║                                                                  ║
 * ║  After Layer 0, evaluate a lightweight "exit head" (32→4).       ║
 * ║  If the top-2 logit margin exceeds a threshold, skip Layers 1   ║
 * ║  & 2 entirely and jump straight to the classification program.   ║
 * ║  Expected benefit: ~85% of benign packets exit early, saving     ║
 * ║  ~40% latency by avoiding 2 tail calls.                         ║
 * ╠══════════════════════════════════════════════════════════════════╣
 * ║  INNOVATION 2 — Ternary Weight Inference (Shift-Add)            ║
 * ║                                                                  ║
 * ║  Hidden-to-hidden layers (1 & 2) use ternary {-α, 0, +α}       ║
 * ║  weights packed as 2 bits each (16 weights per __u32).           ║
 * ║  All multiplications are replaced by conditional add/sub +       ║
 * ║  a single bit-shift per neuron.                                  ║
 * ║  Expected benefit: 0 multiply instructions in hidden layers,     ║
 * ║  ~60% instruction reduction.                                     ║
 * ╠══════════════════════════════════════════════════════════════════╣
 * ║  INNOVATION 3 — Fused Norm-MatMul-ReLU                          ║
 * ║                                                                  ║
 * ║  Layer 0 uses pre-computed fused weights that incorporate        ║
 * ║  input normalization: fused_w0[i][j] = W0[i][j] * norm_scale[j] ║
 * ║  This eliminates the separate normalization pass (12 extra       ║
 * ║  multiplications) and merges normalize + matmul + ReLU into      ║
 * ║  a single loop.  Input features are fed RAW.                     ║
 * ╚══════════════════════════════════════════════════════════════════╝
 *
 * Program layout:
 *   Program 0  (xdp_entry)      — parse packet, extract 12 features
 *   Program 1  (xdp_layer_0)    — fused norm+matmul+ReLU + early-exit check
 *   Program 2  (xdp_layer_1)    — ternary matmul (32→32) + ReLU
 *   Program 3  (xdp_classify)   — ternary matmul (32→4) + argmax + action
 *
...
#include "../include/model_params_v2.h"
...
struct nn_ctx {
    ...
    __s32 early_exit;               /* NEW: 1 if early-exited */
};
...
/*
 * ================================================================
 * INNOVATION 2 — Ternary Dot Product (Shift-Add)
 * ================================================================
 *
...
/*  No normalization — that is fused into Layer 0 (Innovation 3).      */
/* ================================================================== */
SEC("xdp/entry")
int xdp_entry(struct xdp_md *ctx)
{
     ...
     * Innovation 3 (Fused Norm-MatMul-ReLU) handles normalization
     * inside Layer 0 via pre-computed fused weights.
     *
...
/* ================================================================== */
/*  Program 1 — Fused Norm-MatMul-ReLU (12→32) + Early-Exit Check     */
/*                                                                      */
/*  INNOVATION 3: Fused operator                                        */
/*    Instead of separate normalize → matmul → ReLU, this program      */
/*    uses pre-computed fused weights that incorporate normalization:    */
/*      fused_w0[i][j] = W0[i][j] * norm_scale[j]  (offline)          */
/*      fused_offset[i] = Σ_j W0[i][j]*norm_offset[j]  (offline)      */
/*    Runtime is a SINGLE loop:                                         */
/*      sum = Σ_j fused_w0[i][j] * x_raw[j]                           */
/*      h[i] = ReLU( (sum >> FUSED_SCALE_BITS) - offset[i] + bias[i]) */
/*    This eliminates 12 extra multiplications from a separate          */
/*    normalization pass.                                                */
/*                                                                      */
/*  INNOVATION 1: Early-exit check                                      */
/*    After computing the hidden vector, evaluate a lightweight exit    */
/*    head (32→4 linear layer). Compute the margin between the top-2   */
/*    logits. If the margin > EARLY_EXIT_THRESHOLD, the model is       */
/*    already confident — skip Layers 1 & 2, jump to PROG_CLASSIFY.    */
...
int xdp_layer_0(struct xdp_md *ctx)
{
    ...
    /*
     * ── Fused Norm-MatMul-ReLU (Innovation 3) ──────────────────────
     *
     * Compute hidden[i] directly from raw features, no separate
     * normalization pass.  The fused weights already encode the
     * normalization scale factors.
     *
     * Math:
     *   sum = Σ_j fused_w0[i][j] * features_raw[j]
     *   hidden[i] = max(0, (sum >> FUSED_SCALE_BITS) - fused_offset[i]
     *                      + bias_layer_0[i])
     */
    for (i = 0; i < HIDDEN_SIZE; i++) {
        __s64 sum = 0;
        for (j = 0; j < NUM_FEATURES; j++)
            sum += (__s64)fused_w0[i][j] * (__s64)nctx->features[j];

        __s32 v = (__s32)((sum >> FUSED_SCALE_BITS) - (__s64)fused_offset[i])
                  + bias_layer_0[i];
        nctx->hidden[i] = v > 0 ? v : 0;   /* ReLU fused in */
    }

    /*
     * ── Early-Exit Check (Innovation 1) ────────────────────────────
     *
     * Evaluate a lightweight exit head: a 32→4 linear layer using
     * the hidden vector we just computed.  Then check if the model
     * is already confident enough to skip the remaining layers.
     *
     * Confidence metric: margin between the top-2 logits.
     * This avoids softmax entirely — in the integer domain, a large
     * margin between top-2 logits is a reliable confidence signal.
     */
    ...
    for (i = 0; i < NUM_CLASSES; i++) {
        ...
        for (j = 0; j < HIDDEN_SIZE; j++)
            esum += (__s64)exit_weight[i][j] * (__s64)nctx->hidden[j];
        exit_logits[i] = (__s32)(esum >> SCALE_FACTOR_BITS) + exit_bias[i];
    }

    /* Find max logit (max1) and second-max (max2) */
    ...
    /* Normal path: proceed to Layer 1 */
    bpf_tail_call(ctx, &jmp_table, PROG_LAYER1);
    return XDP_PASS;
}

/* ================================================================== */
/*  Program 2 — Ternary Layer 1 (32→32) + ReLU                        */
/*                                                                      */
/*  INNOVATION 2: Ternary weight inference (shift-add)                  */
...
/*    For 32 inputs, each neuron needs 2 packed words.                  */
/* ================================================================== */
SEC("xdp/layer1")
int xdp_layer_1(struct xdp_md *ctx)
{
    ...
    /*
     * ── Ternary MatMul (Innovation 2) ──────────────────────────────
     *
     * For each output neuron i:
     *   acc = ternary_dot(packed_w1[i], input, 32, ALPHA_SHIFT_LAYER_1)
...
/* ================================================================== */
/*  Program 3 — Classification (32→4) + argmax + action                */
/*                                                                      */
/*  If early_exit == 1, skip recomputation — use the pre-stored        */
/*  class from the exit head (Innovation 1).                            */
/*  Otherwise, run ternary matmul for the final layer (Innovation 2).  */
/* ================================================================== */
SEC("xdp/classify")
int xdp_classify(struct xdp_md *ctx)
{
    ...
    if (nctx->early_exit) {
        /*
         * ── Early-Exit path (Innovation 1) ─────────────────────────
         *
         * The exit head in Layer 0 already determined the class.
         * The predicted class was stored in logits[0].  Skip the
        ...
        /*
         * ── Normal path: Ternary Layer 2 (Innovation 2) ────────────
         *
         * Compute logits[i] = ternary_dot(packed_w2[i], hidden, 32,
         *                                  ALPHA_SHIFT_LAYER_2)
